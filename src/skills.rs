...
impl SkillRegistry {
    ...
    fn register_builtin_skills(&self) {
        ...
        let bash_def = SkillDefinition {
            ...
            description: "Execute a command in the local bash shell. Working directory defaults to /home/drakestapleton/workspace. Timeout is 15 seconds. Broad scans of root (find /) are prohibited.".to_string(),
            parameters_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "command": { "type": "string", "description": "The command line string to run" },
                    "cwd": { "type": "string", "description": "Optional working directory, defaults to /home/drakestapleton/workspace" }
                },
                "required": ["command"]
            }),
        };
        self.register(bash_def, |args| {
            ...
            if trimmed.contains("find / ") || trimmed.contains("find / -") || trimmed == "find /" {
                return Err("Safety error: broad scans of root filesystem (find /) are prohibited. Target specific workspace paths under /home/drakestapleton/workspace.".to_string());
            }

            let cwd = args.get("cwd")
                .and_then(|c| c.as_str())
                .unwrap_or("/home/drakestapleton/workspace");

            let target_dir = if std::path::Path::new(cwd).exists() {
                cwd
            } else {
                "/home/drakestapleton"
            };
